...
import com.ihappy.company.domain.dto.request.CompanyInfoAddByRpcReqDTO;
 ...
 */
public class AddCompanyInfoByRpcProcess extends DefaultProcess<CompanyInfoAddByRpcReqDTO, CompanyInfoAddRespDTO> {
    @Autowired
    ...
    @Override
    public void process(Context<CompanyInfoAddByRpcReqDTO, CompanyInfoAddRespDTO> context) {
        CompanyInfoAddByRpcReqDTO companyInfoAddByRpcReqDTO = context.getParam();
        CompanyModel companyModel = BaseinfoCompanyFactory.companyInfoAddByRpcReqDTORpcToModel(companyInfoAddByRpcReqDTO);
        //填充公司业务分类名称
        ...
        baseinfoCompanyPartnerLevel.setCreatedAt(CompanyDateUtil.getDate14Long(new Date()));
        baseinfoCompanyPartnerLevel.setUpdatedAt(CompanyDateUtil.getDate14Long(new Date()));
